...
@interface QNTimerProxy : NSObject
- (instancetype)initWithInterval:(NSTimeInterval)interval
                         repeats:(BOOL)repeats
                    complement:(void (^)(void))complement;
- (void)invalidate;
@end
@implementation QNTimerProxy{
  NSTimer *_timer;
...
@end
@implementation QNUtilities
+(instancetype)instance{
    static dispatch_once_t onceToken;
    static QNUtilities *ut = nil;
    dispatch_once(&onceToken, ^{
        ut = [[QNUtilities alloc] init];
    });
    return ut;
}
-(void)blockRoom:(NSString *)roomId duration:(int)duration{
    ...
    _timer = [[QNTimerProxy alloc] initWithInterval:duration*60 repeats:NO complement:^{
        weakSelf.locked= NO;
        weakSelf.roomId = nil;
    }];
}
